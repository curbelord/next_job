...
LEFT OUTER JOIN (SELECT id_oferta, COUNT(id_demandante) AS candidatos_inscritos FROM inscripcion GROUP BY id_oferta) AS inscritos ON oferta.referencia = inscritos.id_oferta LEFT OUTER JOIN candidatos_preseleccionados ON oferta.referencia = candidatos_preseleccionados.id_oferta LEFT OUTER JOIN candidatos_descartados ON oferta.referencia = candidatos_descartados.id_oferta WHERE oferta.id_seleccionador=" . $_GET['id_seleccionador'] . " AND oferta.eliminada = 0 ORDER BY created_at DESC LIMIT 5";
