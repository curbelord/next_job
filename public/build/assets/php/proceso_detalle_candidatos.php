...
if ($_GET['curriculumsCiegos'] == "SI"){
    ...
    $sql = "SELECT (SELECT COUNT(*) FROM inscripcion WHERE inscripcion.id_oferta=" . $_GET['referencia'] . ") AS 'numero_inscritos', users.id AS 'id_candidato', CONCAT(users.nombre, ' ', users.apellidos) AS 'nombre_candidato', YEAR(CURDATE()) - YEAR(users.fecha_nacimiento) - (RIGHT(CURDATE(), 5) < RIGHT(users.fecha_nacimiento, 5)) AS 'edad_candidato' FROM users INNER JOIN demandante ON users.id = demandante.id LEFT JOIN inscripcion ON demandante.id = inscripcion.id_demandante WHERE inscripcion.id_oferta=" . $_GET['referencia'] . " ORDER BY demandante.checkin DESC LIMIT 10 OFFSET " . $_GET['numero_offset'];
