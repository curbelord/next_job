...
$sql = "SELECT oferta.referencia, oferta.puesto_trabajo, oferta.estado AS 'estado', oferta.ubicacion, oferta.created_at, COUNT(inscripcion.id_demandante) AS 'candidatos_inscritos', candidatos_preseleccionados AS 'candidatos_preseleccionados', curriculums_ciegos, (SELECT COUNT(*) FROM oferta WHERE oferta.id_seleccionador=" . $_GET['id_seleccionador'] . " AND oferta.eliminada = false) AS 'cantidad_total_ofertas' FROM oferta LEFT JOIN inscripcion ON oferta.referencia = inscripcion.id_oferta LEFT JOIN candidatos_preseleccionados ON oferta.referencia = candidatos_preseleccionados.id_oferta  WHERE oferta.id_seleccionador=" . $_GET['id_seleccionador'] . " AND oferta.eliminada = false GROUP BY oferta.referencia, oferta.puesto_trabajo, oferta.ubicacion, oferta.created_at, candidatos_preseleccionados, curriculums_ciegos ORDER BY oferta.created_at DESC LIMIT 10 OFFSET " . $_GET['numero_offset'];
